...
function getTrackStyle(el, oldStyle) {
    ...
    if (id == 'paymnt_terms') val = (curVal - 24) / 12 / el.getAttribute('data-numSteps') * 100;

    if (id == 'intrst_rate') val = (curVal * 100 - 100) / 150 * 100;
...
function updateCalculations() {
    ...
    interestRate = document.getElementById('interest_rate').value / 100;

    monthlyPayment = document.getElementById('monthly_payment');
    ...
    months = paymentTerm;
    monthly_pymnt = interestRate * -loanAmount * Math.pow(1 + interestRate, months) / (1 - Math.pow(1 + interestRate, months));
    ...
    for (let i = 0; i < months; i++) {
        ...
        total += interest;
        principal = monthly_pymnt - interest;
    ...
    monthlyPayment.innerHTML = format(monthly_pymnt);
